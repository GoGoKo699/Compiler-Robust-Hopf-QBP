...
The latest constructive result batches distinct phases generated by a few
shared angles, using one clean qubit and logarithmic dirty work when their
integer coefficients are polynomially bounded. Earlier results characterize
useful dirty workspace,
...
## Latest construction: distinct angles from shared base angles
...
reduction gives $`O(N\log^2 N)`$ T gates at $`L=N`$, using two clean
qubits and $`N/2+O(\log N)`$ dirty qubits. This is a sufficient bound for
...
The next constructive task is to handle heterogeneous phases without assuming
a compact shared-generator representation, or implement the weaker
selected-commutator contract in
