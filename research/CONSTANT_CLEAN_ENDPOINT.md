...
| Turn XOR table insertion into modular addition | The naive residual $`(z\oplus f)-z=f-2(z\mathbin{\&}f)`$ depends on dirty data. A new carry-query construction gives exact $`O((N+m)\log(m+1))`$-T addition using $`O(m+n)`$ dirty work and no clean work; a cheap joint precision clock remains missing. |
...
The [appendix index](constant_clean/README.md) supplies the detailed source, XOR, clock, modular, and phase arguments. These results restrict proposed interfaces; none strengthens the selected whole-frame lower bound beyond $`\Omega(N)`$.
...
At $`m=\Theta(L)`$, this requires both a constant-clean joint clock costing $`O(m+L)`$ T gates and a carry-aware table translation with a charged ledger, or a direct commutator construction that avoids separate implementations. The [modular follow-up](constant_clean/MODULAR_LOOKUP_FOLLOWUP.md) now supplies a zero-clean translation at $`O((N+m)\log(m+1))`$ T cost, leaving a logarithmic overhead and the joint-clock problem. The hypothetical modular-lookup target is $`O(m+\sqrt{Nm}+Nm/K)`$, where $`K`$ is bank capacity **after** other reservations. At $`m=N`$, the bank-balancing choice is one word; it leaves the original hard modular lookup. Even both primitives need a determinant-one/real-rotation reduction and full prefix/suffix composition with the desired total cost. Their algebra alone is not a frame compiler.

The [current progress report](CONSTANT_CLEAN_PROGRESS.md) also retains a two-call phase-bank reduction requiring no extra clean qubit and [one-clean masked-accumulator batching](constant_clean/IDENTICAL_PHASE_BATCHING.md). Identical phases cost $`O((m+L)\log(m+1))`$ T gates; the signed-weight extension also covers distinct angles generated from a few shared base angles. A covering bound explains why bounded coefficients and a fixed number of generators cannot approximate arbitrary tables at the endpoint precision. A converse shows that uniform selected commutators from one common circuit already give full batching with only $`O(\log S)`$ extra precision bits and the same workspace. These are equivalent synthesis targets under their common-circuit hypothesis. The new operator-source compiler instead uses address-dependent sign tables and does not establish the fast arbitrary tensor-batch hypothesis. Charge where the classical table actually enters. Verify the actual inverse on every work state and the joint dirty/reference action before optimizing its asymptotics.

## Success criteria and stopping claims

A constructive resolution must prove a uniform $`O(N)`$-T family with a specified constant clean allocation, $`\Theta(N)`$ dirty allocation, and the complete error contract. The new $`O(N\log N)`$ construction achieves intermediate progress; removing the remaining logarithm or proving a stronger unrestricted lower bound is the next frontier. Alternatively, a stronger lower bound must apply to unrestricted admissible frame circuits, not sum costs of a chosen source, clock factorization, or correction architecture.
