...
| Frame-safe recompilation preserves complete global protocol distribution | Algebraic reducing-subspace substitution theorem | `FRAME_SAFE_COMPILATION.md` | Requires complete system action on clean workspace |
...
| Normalized diagonal with `2n<=w<=N/n` has clean depth `O(log w+N/w)` and size `O(N)` | Imported Sun et al. Lemmas 10 and 20 | `complex_resources.py`, common-workspace document | Exact synthesis theorem imported |
| Normalized diagonal has no-ancilla depth `O(N/n)` and size `O(N)` | Imported Sun et al. Lemma 11 | same | Exact synthesis theorem imported |
...
| Ancillary diagonal registers return to zero | Imported construction cleanup | Sun et al. Stage 5 / Lemma 19 | Source theorem, not reimplemented gate by gate |
