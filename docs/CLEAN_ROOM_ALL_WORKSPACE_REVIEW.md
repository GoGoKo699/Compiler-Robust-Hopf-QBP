...
For $m\geq 1$, each nonfinal addressed layer may instead compute the complete
...
When $1\leq m<4n$, $n+m<5n$ and $n^3=O\!\left(2^n\right)$, so the polynomial term is absorbed
by $\frac{2^n}{n+m}$.
...
clean workspace qubits, depth $O\!\left(t\right)$, and size $O\!\left(B\right)$. On the one-hot code, all
...
uncomputed. The complete prefix has depth $O\!\left(n\right)$ and size $O\!\left(B+s\right)$.
...
where $B=2^t$, $t\geq 1$, and $s\geq 2$. Thus the branch flags reuse existing wires
...
For $m\geq 4n$, choose the largest $t$ satisfying
...
prefix and route--unroute stages have $O\!\left(n\right)$ depth. Feasibility of $t=n-1$
requires
...
so $\frac{N}{n+m}=O\!\left(1\right)$ and the target depth is $\Theta\!\left(n\right)$. The routed schedule again
matches it.
...
Together with the strict-zero and direct schedules, this covers every $m\geq 0$.

## 10. Matching lower bounds

Applying the frame to $\lvert 0^n\rangle$ prepares an open family of real normalized states
of dimension $2^n-1$.
...
stream uses $O\!\left(log n\right)=O\!\left(log log M\right)$ executions for $M=\Theta\!\left(2^n\right)$ coordinates.
...
The reconstruction found no reason to retain an $m\geq 1$ exception or to weaken
