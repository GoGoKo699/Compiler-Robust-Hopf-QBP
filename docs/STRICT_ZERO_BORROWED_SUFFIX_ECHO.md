...
All predicate and linear-width terms sum to $O\!\left(n^2\right)$, and
...
$\Omega\!\left(N\right)$ size and, on exactly $n$ wires, $\Omega\!\left(\frac{N}{n}\right)$ depth. These lower
...
$O\!\left(N\right)$ size and $O\!\left(n+\frac{N}{n}\right)$ depth with zero ancillary qubits. Sequential
