...
Two width-$d+2$ UCGs contribute $O\!\left(2^d\right)$ size. Four predicate toggles contribute
$O\!\left(n-d\right)$ size. Two target echoes contribute constants. Hence
...
matching $\Omega\!\left(2^n\right)$ lower bound.
...
depth up to a constant factor. The predicate toggles contribute $O\!\left(n-d\right)$.
Therefore
...
The linear-width and predicate terms total $O\!\left(n^2\right)$, and
...
$O\!\left(nD\right)$ continuously parameterized one-qubit locations. Covering the
...
is one exact $n$-qubit UCG. At zero workspace it has $O\!\left(2^n\right)$ size and
$O\!\left(n+\frac{2^n}{n}\right)$ depth. Therefore
