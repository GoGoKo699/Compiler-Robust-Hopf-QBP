...
| C3 | Lemma 6: a total-width-$q$ UCG with $w$ clean work qubits has $O\!\left(2^q\right)$ size and $O\!\left(q+\frac{2^q}{q+w}\right)$ depth | half-angle UCGs, flagged layers, subtree frames, and the phase diagonal | [`strict_zero_echo.py`](../compiler_robust_hopf/strict_zero_echo.py), [`unified_compiler.py`](../compiler_robust_hopf/unified_compiler.py) |
...
| Q4 | the primary finite-shot target is simultaneous absolute accuracy of the raw coordinate gradient | defines the $O\!\left(\log n\right)$ execution statement | [QBP consequence, Section 5](QBP_CONSEQUENCE.md#5-statistical-target) |
...
| R4 | the strict-zero frame has $\Theta(N)$ size and $\Theta(n+\frac{N}{n})$ depth | [compiler theorem, Proposition Z](COMPILER_THEOREM.md#proposition-z-strict-zero-resources) | [`strict_zero_audit.py`](../compiler_robust_hopf/strict_zero_audit.py), exact-rational tests |
...
| R6 | a clean binary–one-hot decoder has $3\cdot2^t-2-t$ workspace, $O\!\left(t\right)$ depth, and $O\!\left(2^t\right)$ size | [compiler theorem, Lemma P](COMPILER_THEOREM.md#lemma-p-clean-binaryone-hot-decoder) | [`tree_decoder.py`](../compiler_robust_hopf/tree_decoder.py), [`test_tree_decoder.py`](../tests/test_tree_decoder.py) |
