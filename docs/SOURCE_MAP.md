...
|---|---|---|---|
| C1 | Theorem 2: exact arbitrary state preparation has `Theta(2^n)` size and `Theta(n+2^n/(n+m))` depth for every clean-workspace budget | comparison frontier and target lower-bound scale | [compiler theorem](COMPILER_THEOREM.md), [`unified_compiler.py`](../compiler_robust_hopf/unified_compiler.py) |
...
| C3 | Lemma 6: a total-width-`q` UCG with `w` clean work qubits has `O(2^q)` size and `O(q+2^q/(q+w))` depth | half-angle UCGs, flagged layers, subtree frames, and the phase diagonal | [`strict_zero_echo.py`](../compiler_robust_hopf/strict_zero_echo.py), [`unified_compiler.py`](../compiler_robust_hopf/unified_compiler.py) |
...
| H3 | real nonfinal angles use `[0,pi/2]`, the final real depth uses `[0,2pi)`, and complex magnitudes use `[0,pi/2]` | canonical chart domains | [`canonical_magnitude_angle_mask`](../compiler_robust_hopf/frames.py), [`test_frames.py`](../tests/test_frames.py) |
| H4 | the oriented incoming amplitude obeys `partial_(theta_j)|psi>=a_j|e_j>` and `g_(j,j)=a_j^2` | differential geometry | [`RealTreeData.incoming_amplitude`](../compiler_robust_hopf/frames.py), [`test_complex_analysis.py`](../tests/test_complex_analysis.py) |
| H5 | on the canonical domains `a_j>=0`, so `a_j=sqrt(g_(j,j))` | canonical-domain consequence | [`test_frames.py`](../tests/test_frames.py) |
...
| H9 | the phase-dressed complex magnitude frame is `W_(C,mag)=D_ph W_R` | complex magnitude interface | [`complex_magnitude_frame_matrix`](../compiler_robust_hopf/frames.py) |
...
| R3 | the borrowed-suffix echo implements one nonfinal addressed layer with no ancillary wire | [compiler theorem, Lemma 3](COMPILER_THEOREM.md#lemma-3-borrowed-suffix-echo) | [`strict_zero_echo.py`](../compiler_robust_hopf/strict_zero_echo.py), [`test_strict_zero_echo.py`](../tests/test_strict_zero_echo.py) |
| R4 | the strict-zero frame has `Theta(N)` size and `Theta(n+N/n)` depth | [compiler theorem, Proposition 4](COMPILER_THEOREM.md#proposition-4-strict-zero-resources) | [`strict_zero_audit.py`](../compiler_robust_hopf/strict_zero_audit.py), exact-rational tests |
| R5 | the prefix is a conditioned smaller frame and the tail is a direct sum of subtree frames | [compiler theorem, Lemmas 6–7](COMPILER_THEOREM.md#6-exact-tree-cut) | [`tree_structure.py`](../compiler_robust_hopf/tree_structure.py), cut tests |
| R6 | a clean binary–one-hot decoder has `3*2^t-2-t` workspace, `O(t)` depth, and `O(2^t)` size | [compiler theorem, Lemma 8](COMPILER_THEOREM.md#lemma-8-clean-binaryone-hot-decoder) | [`tree_decoder.py`](../compiler_robust_hopf/tree_decoder.py), [`test_tree_decoder.py`](../tests/test_tree_decoder.py) |
| R7 | an explicit coherent router realizes the tail direct sum and clears all data, token, copy, and flag work registers | [compiler theorem, Lemma 9](COMPILER_THEOREM.md#lemma-9-explicit-coherent-router) | [`router.py`](../compiler_robust_hopf/router.py), [`test_router.py`](../tests/test_router.py) |
| R8 | the maximal feasible cut attains `O(n+N/(n+m))` depth for every large workspace, including `s=1` | [compiler theorem, Proposition 10](COMPILER_THEOREM.md#proposition-10-maximal-cut-depth) | [`resource_bounds.py`](../compiler_robust_hopf/resource_bounds.py), broad-grid tests |
...
| R10 | the real frame and phase-dressed complex magnitude frame attain the all-workspace optimum | [compiler theorem, Theorem 2](COMPILER_THEOREM.md#3-main-theorem) | [`unified_compiler.py`](../compiler_robust_hopf/unified_compiler.py), both resource ledgers |
