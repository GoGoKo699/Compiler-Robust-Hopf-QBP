...
and let $m\geq0$ be the number of clean ancillary qubits.  The exact logical
...
On the canonical Hopf domains, $a_j\geq0$ and equals the principal metric square
...
An exact two-qubit example preserves $W\lvert0^n\rangle$ while moving two marker columns
