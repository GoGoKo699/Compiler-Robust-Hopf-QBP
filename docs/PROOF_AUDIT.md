...
`|a_j|`. On the canonical Hopf domains, $a_j\geq 0$ and therefore
$a_j=\sqrt{g_{j,j}}$.
...
and summation gives $\Theta\!\left(N\right)$ size and $\Theta\!\left(n+\frac{N}{n}\right)$ depth at $m=0$.
...
depth $11t-4=O\!\left(t\right)$, and implement
...
For $m\geq 4n$, the largest cut satisfying
...
overhead is $O\!\left(log n\right)=O\!\left(log log M\right)$. This excludes output materialization and
