...
Let $N=2^n$.  For every clean-workspace budget $m\geq 0$, the real Hopf
...
$a_j\geq 0$ and equals the principal metric square root.
...
5. **Strict-zero optimality.** The complete real frame has $\Theta\!\left(N\right)$ size and
   $\Theta\!\left(n+\frac{N}{n}\right)$ depth.
...
8. **Binary–one-hot decoder.** The prefix frame is implemented in $O\!\left(t\right)$ depth
   and $O\!\left(2^t\right)$ size.
...
factor and the execution overhead is $O\!\left(log n\right)=O\!\left(log log M\right)$.  The statement is
