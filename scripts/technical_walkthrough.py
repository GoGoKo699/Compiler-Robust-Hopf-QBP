#!/usr/bin/env python3
"""Ten-step technical walkthrough of the repository's central identities.

This script is an orientation tool. It checks representative exact finite
identities and integer resource conditions in the same order as REVIEW.md. It
does not replace the analytic proofs.
"""
...
def step_chart_domain_and_singularity() -> str:
    ...
    require(
        ...
        "singular marker column is not a unit frame direction",
    )
    return "canonical a_j=sqrt(g_jj); singular derivative=0 with a unit marker direction"
...
def main() -> int:
    checks: tuple[tuple[str, Callable[[], str]], ...] = (
        ...
        ("chart domain and singular frame direction", step_chart_domain_and_singularity),
    ...
    print("Optimal Hopf-frame compiler: technical walkthrough")
    print("=" * 54)
    failures = 0
