...
number of clean copies needed at that level is $2^j\,w-1$.  Summing over
$j=0,\ldots,t-1$ gives
...
but not depth.  Since $B\,2^s=N$, the total size remains $O(N)$.
...
Attach one phase $\phi_l$ to each computational leaf:
